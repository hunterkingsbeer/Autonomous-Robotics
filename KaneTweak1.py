    ...
    while currentTileNum != desiredTile:  # until you find the desired tile
        ...
        # if desired tile is WITHIN the current row, need to travel LEFT or RIGHT
            ...
            # if the desired tile is to the RIGHT of the robot
                ...
                changeOrientation(90, False)  # face right
                countBlackTile()

    #tank_drive.on_for_rotations(SpeedPercent(15), SpeedPercent(15), 0.2)
    # announce when you findd the desired tile
...
def scanColumn(columnNumber):
    ...
    if columnNumber != 2:  # if column 1 or 3 (with 2 black tiles on the edge of the big tile)
        #ctank_drive.on_for_rotations(SpeedPercent(20), SpeedPercent(20), 0.2)  # drive to center of tile
        tankRotateRight(90)  # turn right, to face down (180) to scan
        tempDist = ultrasonic()
        tankRotateLeft(90)  # reverse the turn
        # tank_drive.on_for_rotations(SpeedPercent(20), SpeedPercent(20), -0.2)
...
def seekTower():
    for x in range(1, 4):
        ...
        if scanColumn(x):
            announce("found tower, dist " + str(int(towerDist)))
...
def correction():
    ...
    if orientation == 180 or orientation == 0:  # set multiplier for orientation correctly
        multiplier = 0.38  # needs to be less to account for further distance between black tiles when travelling down
